<template>
  <div class="fill-height d-flex flex-column">
    <v-btn icon large class="ma-2" to="/">
      <v-icon>
        mdi-arrow-left
      </v-icon>
    </v-btn>
    <v-container class="grow d-flex flex-column align-center justify-center">
...
export default {
  ...
  methods: {
    async submit() {
      ...
      try {
        ...
        localStorage.setItem('last-auction-options', JSON.stringify(auctionOptions));
  ...
  created() {
    const auctionOptionsString = localStorage.getItem('last-auction-options');
